...
import { FileText, Youtube, Clock, Sparkles, Plus, ArrowUpRight, Loader2, Search } from 'lucide-react';
  ...
  return (
    <div className="min-h-full flex flex-col relative pb-32 md:pb-20">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center pt-8 md:pt-24 pb-8 md:pb-16 px-4 md:px-4">
        ...
        >
           <div className="absolute inset-0 bg-gradient-to-r from-primary/0 via-primary/10 to-primary/0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-xl hidden md:block" />
           ...
           {/* Chips */}
           <div className="flex md:justify-center gap-3 mt-4 md:mt-6 overflow-x-auto scrollbar-hide pb-2 md:pb-0 px-1 md:px-0">
              {['Machine Learning basics', 'Introduction to Python', 'History of Art'].map((text, i) => (
                <motion.button
                  key={i}
                  ...
                  className="px-4 py-2 rounded-full border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] text-sm text-white/60 hover:text-white whitespace-nowrap transition-all duration-200 hover:border-white/20 hover:shadow-md shrink-0"
                >
                  {text}
                </motion.button>
              ))}
